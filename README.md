...
| `date` | `string\|date, string\|array` | `date` | Creates a new `Date` with the given value. If the date happens to be parseable as a raport date literal, with or without the `#`s, it will parse with the raport date literal parser. The second argument can be used to speciffy the time for the resulting date, including time zone. If given as a string, it will be parsed with the exact date parser. |
