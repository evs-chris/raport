...
| `in` | `any\|array, array\|string\|DateRange\|range\|any` | `boolean` | Returns true if the given `array\|string` contains the given value using `indexOf`. If the target is a date range, it will check to see if the value as a date is in the range. If the value is an array, it will check for every value in the source in the target array, returning `true` if all are found. If the target is a range string e.g. '1, 3, 5, 15-25, <999' and the value is number-like, the value will be checked against the range to see if it matches a member. If the target is anything else, it will check for equality. |
