...
| `deep-is-not` | `any, any, 'strict'\|'loose'\|compare` | `boolean` | Do a deep inequality check on the first two arguments using `'loose'` equality for primitives by default. If used in call form, the third argument can be supplied and can be the strings `'loose'` or `'strict'` or an application that will be passed the two values being compared. The `===` alias automatically uses the `'loose'` equality mode. |
| `detect-delimiters` | `string, number = 2048` | `CSVOptions` | Detect delimited text delimiters based on the first chunk of the data (by default 2048 characters). The resulting object can be passed to `parse` with the `csv` flag to parse delimited text. The detected values are field and record delimiters and field quotes. |
...
| `parse` | `string, opts` | `any` | Parses the given string with a Raport parser, which can be controlled by the options given. If any of `date`, `template`, `time`, `expr`, `schema`, or `csv` is truthy, then that parser will be used. Otherwise, the default Raport parser will be used, so to use the `schema` parser, you could call `parse('string|number' schema:1)`. The `csv` parser accepts delimited parsing options as well as a `detect` option that will cause it to guess at what parsing config to use on the target string. |
