...
// TODO: allow widget to add css by id to html
...
registerRenderer<Label>('label', (w, ctx, placement, state) => {
  ...
  const str = (Array.isArray(w.text) ? w.text : [w.text]).map(v => {
    if (typeof v === 'object' && 'text' in v) return `<span style="${styleFont(v.font)}">${evaluate(ctx, v.text)}</span>`;
    else return evaluate(ctx, v);
  }).join('');
  return `<span class="label" style="${style(w, placement, ctx)}">${escapeHTML(str)}</span>`
});
...
registerRenderer<Container>('container', (w, ctx, placement, state) => {
  ...
  r.output = `<div class="container" style="${style(w, placement, ctx, { computedHeight: h || r.height, container: true })}">${r.output}</div>`;
  r.height = h || r.height;
  return r;
}, { container: true });
...
registerRenderer<Repeater, RepeatState>('repeater', (w, ctx, placement, state) => {
  ...
  if (!state || !state.state || state.state.part !== 'footer') {
    for (let i = (state && state.state && state.state.current) || 0; i < arr.length; i++) {
      ...
      if (r.height > available) return { output: `<div class="container repeat" style="${style(w, placement, ctx, { computedHeight: y, container: true })}">\n${html}</div>`, height: y, continue: { offset: y, state: { part: 'body', src, current: i } } };
      else available -= r.height;

      html += r.output;
      y += r.height;

      if (group && r.continue) return { output: `<div class="container repeat" style="${style(w, placement, ctx, { computedHeight: y, container: true })}">\n${html}</div>`, height: y, continue: { offset: y, state: { part: 'body', src, current: i }, child: r.continue } };
    }
  }
  ...
  return { output: `<div class="container repeat" style="${style(w, placement, ctx, { computedHeight: y, container: true })}">\n${html}</div>`, height: y };
}, { container: true });

registerRenderer<Image>('image', (w, ctx, placement, state) => {
  addStyle(ctx, 'image', `.image { position: absolute; }`);
  return `<img class="image" src="${evaluate(ctx, w.url)}" style="${style(w, placement, ctx)}" />`;
});
...
registerRenderer<MeasuredLabel>('measured', (w, ctx, placement, state) => {
  ...
  if (!state && height > placement.availableY) {
    ...
    return {
      height, output: `<span class="label" style="display:inline-block;white-space:pre-wrap;line-height:${(w.font && w.font.line) || (w.font && w.font.size) || 1}rem;${style(w, placement, ctx, { computedHeight: height, container: true })}">${escapeHTML(text)}</span>`
    };
  }
});
